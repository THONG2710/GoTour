...
const History: React.FC<HistoryProp> = props => {
  ...
  const [hst, setHistory] = useState<TourModel[]>();
    ...
    const fetchData = async () => {
      try {
        const url =
          'http://' +
          ID_HOME_A +
          ':3000/api/user/getTourByIdUser?id=' +
          user._id;
    ...
    <FlatList
      ...
      renderItem={({item}) => (
        <View style={styles.boxHistory}>
          <Image style={styles.img} source={{uri: item.images.toString()}} />
          ...
          <View style={styles.boxMain}>
            ...
            <View
              ...
              <Text style={styles.txtTime}>
                {item.numberOfDays.toString()}N{item.numberOfNights.toString()}Đ
              </Text>
            </View>
            <Text style={styles.txtTurn}>
              {item.favorites.toString()} lượt đi
            </Text>
