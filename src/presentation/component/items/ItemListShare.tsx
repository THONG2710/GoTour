...
const ItemListShare: React.FC<ItemListShareProps> = props => {
  ...
  const getImageUri = () => {
    return imageLike ? ICON_LIKED : ICON_LIKE;
  };

  useEffect(() => {
    const fetchData = async () => {
      const user = await getData(
        'http://' + ID_HOME_A + ':3000/api/user/getUserById?id=' + item.user,
      );
      const tour = await getData(
        'http://' + ID_HOME_A + ':3000/api/tour/getTourById?id=' + item.tour,
      );
      setUser(user.user);
      setTour(tour.tour);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.Poster}>
          <Image style={styles.imgPoster} source={{uri: user?.avatar}}></Image>
          <View>
            <Text style={[styles.text, {fontSize: 16}]}>{user?.userName}</Text>
        ...
        <TouchableOpacity>
          <Image
            ...
            source={{uri: ICON_MENU_POINT}}></Image>
        </TouchableOpacity>
      </View>
      {/* CENTER */}
      ...
      <View style={styles.center}>
        ...
        <Image
          ...
          source={{uri: item.images.toString()}}></Image>
        <Ripple>
          <View style={styles.groupText}>
            ...
            <Text>{tour?.favorites.toString()}</Text>
            <View
